...
class Game {
    ...
    checkComplete(positions) {
        return this.board.everyPosition(positions,
            (piece) => piece == this.players[0].piece
        ) ||
            this.board.everyPosition(positions,
                (piece) => piece == this.players[1].piece
            )
    }
}
...
new Vue({
    ...
    template: `
            ...
            <div class='board'>
                <div
                    v-for="cell in game.board.getAllCells"
                    class="cell"
                    @click="click(cell.position)"
                >
                    <span v-if="cell == null"></span>
                    <span v-if="cell.type == 'x'">X</span>
                    <span v-else-if="cell.type =='z'">0</span>
                </div>
            </div>
        </grid>
    `
